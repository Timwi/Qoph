...
namespace PuzzleStuff.BombDisposal
{
    static class Segmented
    {
        ...
        public static void GenerateHtml()
        {
            ...
            for (var i = 1; i <= 8; i++)
                General.ReplaceInFile(@"D:\c\PuzzleStuff\DataFiles\Bomb Disposal Puzzle Hunt\Segmented\Segmented.html",
                    $"<!--%{i}-->",
                    $"<!--%%{i}-->",
                    $"<img src='data:image/jpeg;base64,{Convert.ToBase64String(File.ReadAllBytes($@"D:\c\PuzzleStuff\DataFiles\Bomb Disposal Puzzle Hunt\Segmented\{files[i - 1]}.jpg"))}' class='photo' />");
        }
    }
}
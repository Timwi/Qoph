...
thistle	kardo	card	karto	4".Replace("\r", "").Split('\n').Select(line => line.Split('\t')).Select(arr => (eoLeft: arr[1], enLeft: arr[2], eoRight: arr[3], enRight: arr[0], index: int.Parse(arr[4]))).ToArray();
        ...
        public static void Generate_OLD()
        {
            ...
            File.WriteAllText(@"D:\c\PuzzleStuff\DataFiles\Bomb Disposal Puzzle Hunt\One Can Hope\One Can Hope.svg", $@"<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 -10 300 240'>{allSvg}</svg>");
        }
        ...
        public static void Generate()
        {
            ...
            General.ReplaceInFile(@"D:\c\PuzzleStuff\DataFiles\Bomb Disposal Puzzle Hunt\One Can Hope\One Can Hope.html", "<!--%%top-->", "<!--%%%top-->", htmlCodeTop.ToString());
            General.ReplaceInFile(@"D:\c\PuzzleStuff\DataFiles\Bomb Disposal Puzzle Hunt\One Can Hope\One Can Hope.html", "<!--%%bottom-->", "<!--%%%bottom-->", htmlCodeBottom.ToString());
        }
    }
}
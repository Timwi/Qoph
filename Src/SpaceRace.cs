    ...
    static class SpaceRace
    {
        public static void Generate()
        {
...

            {
                ...
                File.WriteAllText($@"D:\c\Qoph\DataFiles\Space Race\Space Race.svg",
                    ...
                    $@"<g>{(Enumerable.Range(0, pairs.Length).Zip(cluephrase.Split(2), (ix, b) => (names: pairs[ix], decoy: pairs[ix].Select(name => decoys.Get(name, null)).ToArray(), clueLetters: b)).Select(tup =>
                    {
                        ...
                        return
                            ...
                            makePath(tup.names[0], loc1.X, loc1.Y, textColor: tup.decoy[0] == null ? "red" : decoys.Count(p => p.Value == tup.decoy[0]) == 1 ? "#0000ff" : "black") +
                            makePath(tup.names[1], loc2.X, loc2.Y, textColor: tup.decoy[1] == null ? "red" : decoys.Count(p => p.Value == tup.decoy[1]) == 1 ? "#0000ff" : "black");
                    }).JoinString())}
                    </g>" +
                $@"</svg>");
            }
